...
const Udemy = (props) => {
    ...
    const confirmInput = () => {
        ...
        if (isNaN(chosen) || chosen <= 0 || chosen > 99) {
            Alert.alert('Invalid number', 'number must be between 1 and 99', [{text: 'Okay', style: 'destructive', onPress: resetInput}])
            return
        }
        setConfirm(true)
        setVal('')
        ...
        confirmedOutput = (
          <Animated.View style={{ opacity: animation, padding: 10 }}>
            <UdemyCard style={{ alignItems: "center" }}>
              ...
              <MainButton onPress={() => { props.startGame(selectedNum)}}>START GAME</MainButton>
            </UdemyCard>
          </Animated.View>
        );
    }
